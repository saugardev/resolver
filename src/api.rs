...
use crate::types::{
    FetchWithReceipt, ProductRequest, ProductResponse, ProductRoute, ProvenanceConsent, Receipt,
...
pub async fn snapshot_source(
    ...
    .await?;
    let pending = fetcher
        .prepare_product_fetch_with_auth(request, ProductRoute::Snapshot, Some(&auth_context))
        .await?;
...
async fn prepare_and_capture_product(
    ...
    .await?;
    let pending = fetcher
        .prepare_product_fetch_with_auth(payload, route, Some(auth_context))
        .await?;
    capture_product_route(
        credits,
        auth_context,
